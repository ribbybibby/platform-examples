...
func Map(m mapper.Mapper, input []byte) ([]byte, error) {
	...
	for _, child := range res.AST.Children {
		...
		case "from":
			...
			img, err := mapImage(m, from)
			if err != nil {
				log.Printf("WARN: error mapping image: %s", err)
				continue
			}

			replacement = strings.ReplaceAll(child.Original, child.Next.Value, img)
		...
		case "run":
			...
			for _, flag := range child.Flags {
				...
				match := fromRe.FindStringSubmatch(flag)
				if len(match) < 2 {
					continue
				}
				from := match[1]
...
// mapImage maps an image to its Chainguard equivalent
func mapImage(m mapper.Mapper, img string) (string, error) {
	mapping, err := m.Map(img)
	if err != nil {
		return "", fmt.Errorf("mapping image: %s: %w", img, err)
	}
	if len(mapping.Results) == 0 {
		return "", fmt.Errorf("no results for : %s", img)
	}

	// Default to the -dev tag, as its more likely to work ootb
	result := mapping.Results[0]
	if !strings.HasSuffix(result, "-dev") {
		result = fmt.Sprintf("%s-dev", result)
	}

	return result, nil
}

// fromRe extracts images in `from=` options in `RUN --mount` instructions
var fromRe = regexp.MustCompile(`\bfrom=([^,]+)`)

// argPattern identifies
