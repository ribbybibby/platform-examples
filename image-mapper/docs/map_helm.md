...
It requires that `helm` is available locally.
...
You can optionally specify the chart repository and/or the chart version.
...
This does not require that `helm` is available locally. This will only identify
values that are explicitly listed in the file and may not include all the
values that are provided by subcharts or dependencies.
